...
def add_restaurant():
    try:
        ...
        if existing_reastaurant:
            ...
            new_Restaurant = Restaurant(name=Restaurant_name, image=Image , Status = Status)
            db.session.add(new_Restaurant)
            db.session.commit()
            # Получаем product_id после вставки новой записи
            reastaurant_id = new_Restaurant.id

        return jsonify({'message': 'Restaurant added successfully', 'new_Restaurant_id': reastaurant_id})
    except Exception as e:
        # Логируем ошибку
